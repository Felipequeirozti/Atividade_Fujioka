...
import Atividade_Fujioka.service.DiretorService;
...
public class DiretorController {
    ...
    private final DiretorService filmeService;
    ...
    public ResponseEntity<DiretorDTO> buscarPorId(@PathVariable Long id) {
        ...
        Optional<DiretorDTO> filme = diretorService.buscarPorId(id);
        return diretor.map(ResponseEntity::ok)
                .orElseGet(() -> ResponseEntity.notFound().build());
    }

    @PostMapping
    public ResponseEntity<DiretorDTO> criarFilme(@RequestBody   DiretorDTO diretorDTO) {
